...
class Level2Scene: SKScene {
    ...
    // Boss textures.
    let bossTexture1 = SKTexture(imageNamed: "Boss2-1.png")
    let bossTexture2 = SKTexture(imageNamed: "Boss2-2.png")
    let bossTexture3 = SKTexture(imageNamed: "Boss2-3.png")
    ...
    func createBackground() {
        GameplayCore().background(CGSize(width: self.frame.width, height: self.frame.height), frame: self.frame, backgroundImageName: "lvl1", layer: baseLevel.backgroundLayerNode)
    ...
    func levelOver(playerWin: Bool){
        ...
        let runAction = SKAction.runBlock { () -> Void in
            ...
            let scene = MidGameHubScene(_size: CGSize(width: self.frame.width, height: self.frame.height), _score: self.baseLevel.score, isWin: playerWin, _levelName: 2)
    ...
    func createBoss(){
        ...
        //baseLevel.boss = GameplayCore().boss(CGSize(width: self.frame.width, height: self.frame.height), bossType: 2)
